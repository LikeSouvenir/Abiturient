...
if ($cluster_id_filter) {
    ...
    if ($cluster_data = $cluster_name_res->fetch_assoc()) {
        $current_cluster_name = $cluster_data['name'];
        $page_title = "Профессионалитет: " . htmlspecialchars($current_cluster_name);
    } else {
    ...
    $stmt_links->bind_param("i", $cluster_id_filter);
    $stmt_links->execute();
    ...
    $page_title = "Профессионалитет: Кластер не выбран";
}
...
<body>
    <header class="header">
        ...
        <div class="search-area">
            <div class="search-container">
                ...
                <input type="text" class="search-input" id="linkSearchInput" placeholder="Поиск по колледжам и программам...">
            </div>
    ...
    <main class="container">
        <div class="left-column">
            ...
            <div class="link-list" id="linkList">
                <?php if (!$cluster_id_filter): ?>
                    <p class="no-results php-message">Кластер не выбран. Пожалуйста, вернитесь на главную страницу и выберите кластер из раздела "Профессионалитет".</p>
                <?php elseif (empty($links_data)): ?>
                    <p class="no-results php-message">В данном кластере пока нет доступных программ.</p>
                <?php else: ?>
        ...
        <div class="right-column">
            <h3 class="map-title">Карта учебных заведений кластера</h3>
            <div id="map" class="map-container">
                <?php if (!$cluster_id_filter || empty($links_data) || !array_filter($links_data, function($link_item) { return !empty($link_item['latitude']) && !empty($link_item['longitude']) || !empty($link_item['map_address']); })): ?>
    ...
    const linksDataForMap = <?php echo ($cluster_id_filter && !empty($links_data)) ? json_encode(array_values($links_data), $json_options) : '[]'; ?>;
    ...
    function initMap() {
        ...
        try {
            ...
            linksDataForMap.forEach((link) => {
                ...
                let balloonContent = `<h3>${collegeName}</h3>`;
                ...
                function addPlacemarkDataToList(coords, currentLinkData) {
                     allPlacemarksData.push({
                        ...
                        properties: { balloonContentHeader: collegeName, balloonContentBody: balloonContent, hintContent: collegeName },
                        options: {}
                    });
                }
