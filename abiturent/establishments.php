...
if ($program_code_filter) {
    ...
    $stmt_prog_name = $conn->prepare("SELECT name FROM programs WHERE program_code = ?");
    $stmt_prog_name->bind_param("s", $program_code_filter);
    $stmt_prog_name->execute();
    $prog_name_res = $stmt_prog_name->get_result();
    if ($prog_data = $prog_name_res->fetch_assoc()) {
        $display_program_name = $prog_data['name'];
        $page_title = "Колледжи по программе: " . htmlspecialchars($display_program_name);
    }
    if ($prog_name_res) $prog_name_res->close();
    $stmt_prog_name->close();
    ...
    $stmt_cluster_name = $conn->prepare("SELECT name FROM clusters WHERE id = ?");
    $stmt_cluster_name->bind_param("i", $cluster_id_filter);
    $stmt_cluster_name->execute();
    $cluster_name_res = $stmt_cluster_name->get_result();
    if ($cluster_data = $cluster_name_res->fetch_assoc()) {
        $page_title = "Профессионалитет: " . htmlspecialchars($cluster_data['name']);
    } else {
        $page_title = "Профессионалитет: Кластер не найден";
    }
    if ($cluster_name_res) $cluster_name_res->close();
    $stmt_cluster_name->close();
}

$sql_query = $base_sql_select;
if (!empty($where_clauses)) {
    $sql_query .= " WHERE " . implode(" AND ", $where_clauses);
}
$sql_query .= " ORDER BY e.name, p.name";

$stmt_links = $conn->prepare($sql_query);
if (!empty($bind_params_values)) {
    $stmt_links->bind_param($bind_params_types, ...$bind_params_values);
}
$stmt_links->execute();
...
foreach($raw_bundles_data as $bundle) {
    ...
    if ($cluster_id_filter && !$program_code_filter) {
        $link_item['show_program_name_in_card'] = true;
    }
    ...
    $link_item['search_text'] = strtolower(
        ...
        ($link_item['is_professionalitet'] ? " профессионалитет " . ($bundle['cluster_name'] ?? '') : "")
    );
...
<head>
    ...
    <style>
        ...
        .link-details .program-name-in-card { font-size: 1rem; color: #555; margin-bottom: 6px; display:block; font-weight: normal; }
        ...
        .loading, .no-results { text-align: center; padding: 20px; font-size: 1.2em; color: #555; width: 100%; }
...
<body>
    <header class="header">
        ...
        <div class="search-area">
            <div class="search-container">
                ...
                <input type="text" class="search-input" id="linkSearchInput" placeholder="Поиск по колледжам и параметрам...">
            </div>
                    ...
                    <?php foreach ($links_data as $link): ?>
                        <div class="link-item" data-id="<?php echo $link['id']; ?>" data-search-text="<?php echo htmlspecialchars($link['search_text']); ?>" data-lat="<?php echo htmlspecialchars($link['latitude']); ?>" data-lon="<?php echo htmlspecialchars($link['longitude']); ?>">
                            ...
                            <div class="link-details">
                                <h3><?php echo htmlspecialchars($link['college_name']); ?></h3>
                                ...
                                <div class="tags-container">
                                    <?php if (!empty($link['program_attributes_array'])): ?>
                                        <?php foreach ($link['program_attributes_array'] as $attr):
                                              ...
                                              if (strpos($attr_lower, '2 огэ') !== false) { $tag_class .= ' oge-attribute'; }
                                        ?>
                                    ...
                                    <?php if ($link['is_professionalitet']): ?>
                                        <span class="attribute-tag professionalitet-tag">
                                            Профессионалитет<?php echo !empty($link['cluster_name']) ? ': ' . htmlspecialchars($link['cluster_name']) : ''; ?>
                                        </span>
                                    <?php endif; ?>
                                </div>
                            </div>
    ...
    const linksDataForMap = <?php echo json_encode(array_values($links_data), $json_options); ?>;

    function initMap() {
        const mapElement = document.getElementById('map');
        if (!mapElement) return;

        const noDataMsgMap = mapElement.querySelector('.no-results');
        if(noDataMsgMap) noDataMsgMap.style.display = 'none';
        ...
        try {
            ...
            linksDataForMap.forEach((link) => {
                ...
                const programNameForBalloon = link.program_name_in_bundle || (document.getElementById('pageMainTitle').textContent.startsWith("Колледжи по программе:") ? document.getElementById('pageMainTitle').textContent.replace("Колледжи по программе: ", "") : 'Программа');
                
                let balloonContent = `<b>${collegeName}</b>`;
                balloonContent += `<p><strong>Программа:</strong> ${programNameForBalloon}</p>`;
                ...
                if (link.is_professionalitet) {
                    tagsHtml += `<span class="attribute-tag professionalitet-tag" style="background-color:#fff3cd; color:#856404; padding:2px 5px; border-radius:3px; font-size:0.8em; margin-right:3px;">Профессионалитет${link.cluster_name ? ': ' + link.cluster_name : ''}</span>`;
                }
                if(tagsHtml) balloonContent += `<div style="margin-top:5px;">${tagsHtml}</div>`;

                function addPlacemarkData(coords, currentLinkData) {
                    allPlacemarksData.push({
                        coords: coords,
                        id: currentLinkData.id,
                        properties: {
                            balloonContentHeader: collegeName,
                            balloonContentBody: balloonContent,
                            hintContent: collegeName
                        },
                        options: {}
                    });
                }
                ...
                if (link.latitude && link.longitude) {
                    addPlacemarkData([parseFloat(link.latitude), parseFloat(link.longitude)], link);
                    ...
                    const geocodePromise = ymaps.geocode(link.map_address, { results: 1 })
                        .then(function (res) {
                            ...
                            if (firstGeoObject) {
                                addPlacemarkData(firstGeoObject.geometry.getCoordinates(), link);
                            }
                        }, function(err){
            ...
            Promise.all(geocodingPromises)
                .then(() => {
                    ...
                    if (allPlacemarksData.length > 0) {
                         if(noDataMsgMap) noDataMsgMap.style.display = 'none';
                        yandexMapInstance.setBounds(placemarksCollection.getBounds(), {
                            checkZoomRange: true,
                            zoomMargin: 35
                        });
                    } else {
                        const hasPotentiallyMappableData = linksDataForMap.some(
                            link => (link.latitude && link.longitude) || (link.map_address && link.map_address !== 'Адрес не указан')
                        );
                        if (noDataMsgMap) { noDataMsgMap.style.display = 'block'; }
                ...
                .catch(err => {
                    console.error("Error processing placemarks:", err);
                     if (noDataMsgMap) { noDataMsgMap.style.display = 'none'; } // Hide initial if any
    ...
    function filterLinks(searchTerm) {
        ...
        items.forEach(item => {
            ...
            if (text.includes(term)) { item.style.display = 'flex'; found = true; }
            else { item.style.display = 'none'; }
        });

        const listContainer = document.getElementById('linkList');
        let jsNoResultsMsg = listContainer.querySelector('p.js-no-results-message');
        const phpNoResultsMsg = listContainer.querySelector('p.no-results:not(.js-no-results-message)');


        if (!found && term !== '') {
            if (!jsNoResultsMsg) {
                jsNoResultsMsg = document.createElement('p');
                jsNoResultsMsg.className = 'no-results js-no-results-message';
                listContainer.appendChild(jsNoResultsMsg);
            }
            jsNoResultsMsg.textContent = 'По вашему запросу ничего не найдено.';
            jsNoResultsMsg.style.display = 'block';
            if(phpNoResultsMsg) phpNoResultsMsg.style.display = 'none';

        } else {
            if (jsNoResultsMsg) {
                 jsNoResultsMsg.style.display = 'none';
            }
            if(term === '' && phpNoResultsMsg && linksDataForMap.length === 0){
                 phpNoResultsMsg.style.display = 'block';
            } else if (phpNoResultsMsg){
                phpNoResultsMsg.style.display = 'none';
            }
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        if (typeof ymaps !== 'undefined') { ymaps.ready(initMap); }
            ...
            const noDataMsgMap = mapElement.querySelector('.no-results');
            if (noDataMsgMap) noDataMsgMap.style.display = 'none';
            mapElement.innerHTML = '<p class="no-results" style="padding-top: 40px;">API Яндекс.Карт не загружено.</p>';
        }
        ...
        if(linksDataForMap.length === 0){
            const listContainer = document.getElementById('linkList');
            if(!listContainer.querySelector('p.no-results')){
                 const p = document.createElement('p');
                 p.className = 'no-results';
                 p.textContent = 'Подходящие варианты не найдены.';
                 listContainer.appendChild(p);
            }
        } else {
            const phpNoResultsInList = document.querySelector('#linkList > p.no-results');
            if (phpNoResultsInList) phpNoResultsInList.style.display = 'none';
        }
    });
    </script>
</body>
